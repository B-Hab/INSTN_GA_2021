...
L'étude statistique à mener se base sur le cas 3 présenté pendant la séance 2.\\

Un choix de paramètres d'entrée à probabiliser (i.e. à considérer comme des variables aléatoires) a été réalisé \textit{a priori}. Ces paramètres sont : 
...
\Q{Calculer la probabilité de percemment pour cette étude en modifiant les fichiers \texttt{*.uranie} du répertoire \texttt{data} et en relance l'outil statistique de procor avec le script \texttt{launcher_stat.sh}.}
...
\Q{Choisir un cas par famille identifiée. Pour cela, utiliser la commande \texttt{t->scan("","\text{condition}"}. Lancer le calcul de chacun de ces cas afin de récupérer l'ensemble des résultats (en particulier les fichier CSV) en changeant la verbosité de \texttt{low} à \texttt{high}. Expliquer l'"histoire" de chaque famille et ce qui la caractérise.}
